	...
	class CRest
	{
		...
		public static function setLog($arData, $type = '')
		{
			...
			if(!defined("C_REST_BLOCK_LOG") || C_REST_BLOCK_LOG !== true)
			{
				...
				if (!file_exists($path))
				{
					$old = umask(0); 				   //зайцев - фикс - пхп может игнорирует параметры прав доступа, если объявлен umask
					  @mkdir($path, 0775, true);       //права на каталог (рекурсивные для всех дочерних каталогов)
					umask($old);					   //зайцев 
				}
